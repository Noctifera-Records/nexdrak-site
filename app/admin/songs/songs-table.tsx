...
export default function SongsTable({ songs: initialSongs }: SongsTableProps) {
  ...
  const handleAddSong = async (songData: Omit<Song, 'id' | 'created_at'>) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error desconocido al agregar canción'
      alert(errorMessage)
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSongs.map((song) => (
          <div key={song.id} className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden group hover:border-gray-600 transition-all">
            {/* Imagen de portada */}
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-center justify-end space-x-2 pt-2 border-t border-gray-700">
                <Dialog>
                  ...
                  <DialogContent className="bg-gray-900 border-gray-700 max-w-2xl">
                    <DialogHeader>
                      <DialogTitle className="text-white">Editar Canción</DialogTitle>
                      <DialogDescription className="text-gray-400">
                    ...
                    {editingSong && (
                      <SongForm
                        song={editingSong}
                        onSave={(data) => handleUpdateSong(editingSong.id, data)}
                        loading={loading}
                      />
                    )}
                  </DialogContent>
                ...
                <Dialog>
                  ...
                  <DialogContent className="bg-gray-900 border-gray-700">
                    <DialogHeader>
                      <DialogTitle className="text-white">Eliminar Canción</DialogTitle>
                      <DialogDescription className="text-gray-400">
...
function SongForm({ 
  ...
  const [coverImageUrl, setCoverImageUrl] = useState(song?.cover_image_url || '')
